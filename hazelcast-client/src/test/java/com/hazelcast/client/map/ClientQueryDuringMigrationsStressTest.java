...
public class ClientQueryDuringMigrationsStressTest extends HazelcastTestSupport {
    ...
    public void testMembersWithIndexesAndShutdownStartup()
        ...
        populateMap(map, 300000);
        System.out.println("Done populating map");
//        waitClusterForSafeState(members[0]);

        ExecutorService queryingExecutor = Executors.newFixedThreadPool(CONCURRENT_QUERYING_CLIENTS);

        Future[] queryingFutures = queryContinuously(clients, queryingExecutor, CONCURRENT_QUERYING_CLIENTS);
    ...
    private Future[] queryContinuously(HazelcastInstance[] instances, ExecutorService executor, int concurrency) {
        ...
        for (int i = 0; i < instances.length; i++) {
            futures[i] = executor.submit(new QueryRunnable(instances[0]));
        }
        return futures;
    }
    ...
    public class MembersMonkey
            implements Runnable {
        private final TestHazelcastFactory nodeFactory;
        private final HazelcastInstance[] instances;

        public MembersMonkey(TestHazelcastFactory nodeFactory, HazelcastInstance[] instances) {
