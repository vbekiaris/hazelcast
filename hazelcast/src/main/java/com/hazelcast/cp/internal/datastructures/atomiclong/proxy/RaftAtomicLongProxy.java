...
public class RaftAtomicLongProxy implements IAtomicLong {
    ...
    public CompletableFuture<Long> addAndGetAsync(long delta) {
        ...
        throw new UnsupportedOperationException(); //`return delta == 0 ? invocationManager.query(groupId, op, LINEARIZABLE) : invocationManager.invoke(groupId, op);
    }
    ...
    public CompletableFuture<Boolean> compareAndSetAsync(long expect, long update) {
        throw new UnsupportedOperationException(); //return invocationManager.invoke(groupId, new CompareAndSetOp(objectName, expect, update));
    }
    ...
    public CompletableFuture<Long> getAndSetAsync(long newValue) {
        throw new UnsupportedOperationException(); //return invocationManager.invoke(groupId, new GetAndSetOp(objectName, newValue));
    }

    @Override
    public CompletableFuture<Void> setAsync(long newValue) {
        throw new UnsupportedOperationException(); //return (InternalCompletableFuture) getAndSetAsync(newValue);
    }
    ...
    private CompletableFuture<Long> doAlterAsync(IFunction<Long, Long> function, AlterResultType alterResultType) {
    ...
    public CompletableFuture<Void> alterAsync(IFunction<Long, Long> function) {
        throw new UnsupportedOperationException(); //return (InternalCompletableFuture) doAlterAsync(function, AlterResultType.NEW_VALUE);
    }

    @Override
    public CompletableFuture<Long> alterAndGetAsync(IFunction<Long, Long> function) {
        throw new UnsupportedOperationException(); //return doAlterAsync(function, AlterResultType.NEW_VALUE);
    }

    @Override
    public CompletableFuture<Long> getAndAlterAsync(IFunction<Long, Long> function) {
        throw new UnsupportedOperationException(); //return doAlterAsync(function, AlterResultType.OLD_VALUE);
    }

    @Override
    public <R> CompletableFuture<R> applyAsync(IFunction<Long, R> function) {
        throw new UnsupportedOperationException(); // return invocationManager.query(groupId, new ApplyOp<>(objectName, function), LINEARIZABLE);
    }
    ...
    public ICompletableFuture<Long> localGetAsync(QueryPolicy queryPolicy) {
        ...
        CompletableFuture<Long> localFuture = invocationManager.queryLocally(groupId, new LocalGetOp(objectName), queryPolicy);

        localFuture.whenCompleteAsync((response, throwable) -> {
            if (throwable == null) {
                resultFuture.setResult(response);
            } else {
                CompletableFuture<Long> future = invocationManager.query(groupId, new LocalGetOp(objectName), queryPolicy);
                future.whenCompleteAsync((responseFromQuery, throwableFromQuery) -> {
                    if (throwableFromQuery == null) {
                        resultFuture.setResult(responseFromQuery);
                    } else {
                        resultFuture.setResult(throwableFromQuery);
                    }
                });
            }
        });
